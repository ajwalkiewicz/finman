...
if PRODUCTION and ALLOW_ORIGINS:
...
def calculate_rates_with_base(eur_rates: FixerAPIResponseSuccess, new_base: supported_currencies) -> RatesResponse:
    """
...
def get_exchange_rates(base: supported_currencies = 'PLN'):
    ...
    if cached_data:
        ...
        eur_based_rates = FixerAPIResponseSuccess.model_validate(json.loads(cached_data))
    else:
        ...
        redis_client.setex(
            today_key, 
            CACHE_DURATION, 
            eur_based_rates.model_dump_json()
        )
